...
function loop_material()
    ...
    for i in 1:length(phonon_files)
        ...
        _, mass, _, _ = get_mstar(name, 2)
        println(get_mstar(name, 2), mass)
        mass = geometric_average(mass)
    ...
    df_material = DataFrame([dummy_name, dummy_mass, dummy_eps_ionic, dummy_eps_static], column_names)                
